...
public class ShortestPaths {
    ...
    public void compute(Node origin) {
        ...
        Heap<Node, Double> unProcessed = new Heap<>();

        PathData pd = new PathData(0, null);

        paths.put(origin, pd);
        unProcessed.add(origin, pd.distance);

        while (unProcessed.size() > 0) {
            Node u = unProcessed.peek();
            HashMap<Node, Double> neighbors = u.getNeighbors();

            for(Node w : neighbors.keySet()) {

                // if we are seeing w for the first time, add path details
                if(paths.get(w) == null) {
                    PathData wData = new PathData(paths.get(u).distance + neighbors.get(w), u);
                    paths.put(w, wData);
                    unProcessed.add(w, paths.get(w).distance);
                } else {
                    // we have seen w before, check to see if the path through
                    // //is shorter than the path we have seen before
                    if((paths.get(u).distance + neighbors.get(w)) < paths.get(w).distance) {
                        paths.get(w).distance = paths.get(u).distance + neighbors.get(w);
                        paths.get(w).previous = u;
                    }
                }
            }
            unProcessed.poll();
    ...
    public double shortestPathLength(Node destination) {
        ...
        else if(paths.get(destination).previous == null && paths.get(destination).distance > 0) {
            return Double.POSITIVE_INFINITY;
        } else if((paths.get(destination).previous == null) && (paths.get(destination).distance <= 0)) {
            return 0.0;
        } else {
    ...
    public LinkedList<Node> shortestPath(Node destination) {
        ...
        PathData destPathData = paths.get(destination);
        Node curr = destination;
        LinkedList<Node> shortestPath = new LinkedList<Node>();

        // iterate thru pathdata to create LL of nodes
        while(destPathData.previous != null) {
            shortestPath.addFirst(curr);
            curr = destPathData.previous;
            destPathData = paths.get(curr);
        }
        // origin & destination are equal
        if(destPathData.previous == null) {
            shortestPath.addFirst(curr);
            return shortestPath;
        }
        else{
    ...
    public static void main(String[] args) {
        ...
        sp.compute(origin);
        HashMap<Node, Double> neighbors = origin.getNeighbors();
        ...
        if(destCode == null) {
            System.out.println("Shortest path from " + origin.getId() + ":");
            System.out.println(origin.getId() + ": " + 0.0);
            for (Node entry : neighbors.keySet()) {
                System.out.println(entry.getId() + ": " + neighbors.get(entry));
            }
        } else {
            if(neighbors.get(destination) == null) {
                System.out.println("Path does not exist");
                return;
            }
            System.out.println(origin.getId() + " " + destCode +  " " + neighbors.get(destination));
        }
        // TODO 6:
